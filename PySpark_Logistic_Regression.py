...
$ PYSPARK_PYTHON=$(which python) nohup spark-submit PySpark_Logistic_Regression.py hdfs:/user/gs3170/train.csv hdfs:/user/gs3170/test.csv ./rf_model1 > rf_output1.txt
'''
...
def main(spark, train, test):
    ...
    LR = LogisticRegression(regParam=0.1, elasticNetParam=0.1, featuresCol='scaled_features', labelCol = ylabel,family='binomial')

    pipeline = Pipeline(stages=[assembler, scaler, LR])
    grid = ParamGridBuilder().addGrid(LR.regParam, [0.0001,0.001,0.1,1,10]).addGrid(LR.elasticNetParam, [0.0001,0.001,0.1,0.5,1]).build()
...
if __name__ == "__main__":
    ...
    spark = SparkSession.builder.appName('Py_LR').getOrCreate()
